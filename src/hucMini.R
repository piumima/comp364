...
huc.load <- function(dataSets,
  ...
  ## data. Note that options marked with TODO are not
  ## yet implented - an error will be returned.
  ##
  ## Args:
  ##  dataSets  : Which data set(s) we should load from.
  ##              Note that the only options to dataTypes
  ##              available for datasets: nki, curtis.validation,
  ##              curtis.dicovery, vanvliet are "all", "none" and "exprs".
  ##
  ##              Specify this argument with a vector ofthe names you
  ##              want to load. Valid options are:
  ##                  miniTCGA
  ##                  vanvliet
  ##                  nki
  ##                  curtis.validation
  ...
  ##  dataTypes : A list. Each element corresponds,
  ##              in order to the dataSets specified and should
  ##              be a vector.
  ##              Specificy what you would like to load for each
  ##              dataset, besides the clinical info (which is
  ##              always loaded).
  ##              Supports:
  ...
  ##
  ##
  ## Eventually, simplifying these two arguments to a list would be a good
  ## idea. The present form is to ensure compatability with other code.
  ##
  ## Returns:
  ##  Returns the data structure miniTCGA.

  ## List of permitted datasets.
  permittedDataSets <- list.files(dataDir)
  ## Sanity checks.
  if ("none" %in% dataTypes && length(dataTypes) > 1) {
    stop("Conflicting arguments to huc.load!\nYou cannot specify \"none\" along with other data types.")
  }
  if ("all" %in% dataTypes && length(dataTypes) > 1) {
    stop("Conflicting arguments to huc.load!\nYou cannot specify \"all\" along with other data types.")
  }
  if (!all(dataSets %in% permittedDataSets)) {
    stop("One or more datasets requested is unavailble or mis-specified. Please check your input to huc.load.")
  }
  ## Init return structure
  dat <- list()
  ## Enter a loop over the requested datasets.
  for(dataSet in dataSets) {
    ## Get the directory.
    dataSetDir <- gsub("//", "/", paste(dataDir, "/", dataSet, sep=""))
    ##
    ## Enter loop over adat
    ## If "all" is specified, load everything.
    if (all(dataTypes == "all")) {
      dataTypes <- c("exprs", "cnv", "somatic.curated")
    }
    ## Load clinical
    dat[[dataSet]][["clinical"]] <- readRDS(gsub("//","/", paste(dataSetDir, "/clinical.rds", sep="")))
    dat[[dataSet]][["probe.info"]] <- readRDS(gsub("//","/",paste(dataSetDir, "/probe.info.rds", sep="")))
    ## If "none", stop.
    if(all(dataTypes == "none")) {
      ...
      for(toLoad in dataTypes) {
        ## Check if it's availble. If not, ignore it.
        if (toLoad %in% gsub(".rds", "", list.files(dataSetDir))) {
          ## Load the file and give it the name we want.
          dat[[dataSet]][[toLoad]] <- readRDS(gsub("//","/",paste(dataSetDir, "/", toLoad, ".rds",sep="")))
        }
      }
    }
  }
...
## huc.color.clinical, also by Ali Tofih
huc.color.clinical <- function(clinical) {
  colors <- huc.clinical.colors()

  clinical <- clinical[,which(colnames(clinical) %in% names(colors)), drop=FALSE]
  if (ncol(clinical) == 0) return(NULL)

  colors <- colors[which(names(colors) %in% colnames(clinical))] ## remove variables in colors not in clinical
  clinical <- clinical[,names(colors), drop=FALSE] ## order variables by their order in colors

  for (variable in names(colors)) {
    if (is.logical(clinical[,variable]))
      clinical[,variable] <- colors[[variable]][sign(clinical[,variable])+1]
    else if (is.factor(clinical[,variable]))
      clinical[,variable] <- colors[[variable]][as.character(clinical[,variable])]
    else
      clinical[,variable] <- colors[[variable]][clinical[,variable]]
  }
...
huc.clinical.colors <- function() {
  ...
  heatmap.clinical[["pam50.inhouse"]] <- c(LumA="blue4", LumB="deepskyblue", Basal="firebrick2", Normal="green4", Her2="hotpink2")
  ## Add additional pam50 calling strategies
  heatmap.clinical[["pam50.genefu"]] <- c(LumA="blue4", LumB="deepskyblue", Basal="firebrick2", Normal="green4", Her2="hotpink2")
  heatmap.clinical[["pam50.authors"]] <- c(LumA="blue4", LumB="deepskyblue", Basal="firebrick2", Normal="green4", Her2="hotpink2")
  heatmap.clinical[["pam50.parker"]] <- c(LumA="blue4", LumB="deepskyblue", Basal="firebrick2", Normal="green4", Her2="hotpink2")
  heatmap.clinical[["pam50.aims"]] <- c(LumA="blue4", LumB="deepskyblue", Basal="firebrick2", Normal="green4", Her2="hotpink2")
  heatmap.clinical[["hybrid"]] <- c(erp.lumA="#205595", erp.lumB="#2D8EC2", erp.normL="#319E37", erp.her2E="#A41350", erp.basalL="#AF1226", ern.her2p="#AF4F23", ern.her2n="#629861")
  ...
  heatmap.clinical[["cit"]] <- c(basL="firebrick2",lumA="blue4",lumB="deepskyblue", normL="green4",lumC="hotpink2",mApo="plum3")
